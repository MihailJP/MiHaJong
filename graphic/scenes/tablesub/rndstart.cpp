...
void TableSubsceneBeginning::Render() {
	...
	if (myTimer.elapsed() < timeOffset) {
		if (roundcode > GameStatus::gameStat()->GameLength) {
			ZoomChar(0, _T("延"), -256, 0, 250000);
			ZoomChar(1, _T("長"),    0, 0, 250000);
			ZoomChar(2, _T("戦"),  256, 0, 250000);
		} else if (roundcode == 0) {
			ZoomChar(0, _T("一"), -336, 0, 250000);
			ZoomChar(1, _T("局"), -112, 0, 250000);
			ZoomChar(2, _T("精"),  112, 0, 250000);
			ZoomChar(3, _T("算"),  336, 0, 250000);
		} else {
			ZoomChar(0, _T("オ"), -336, 0, 250000);
			ZoomChar(1, _T("ー"), -112, 0, 250000);
			ZoomChar(2, _T("ラ"),  112, 0, 250000);
			ZoomChar(3, _T("ス"),  336, 0, 250000);
		}
	} else {
		...
		ZoomChar(        2, _T("局")                              ,  256, 500000 + timeOffset, 750000 + timeOffset);
