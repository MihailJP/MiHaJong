...
　Ver2.3.0以降、鳴く面子に赤ドラを含めるかどうかを選択できるようになりました。
考えられる鳴きが複数ある場合は、「チー」「ポン」を押した後鳴きたい牌を2枚選択してください。

#### チーに関して ####

　従来「左チー」「嵌チー」「右チー」にボタンが分かれていましたが、Ver2.3.0以降
これらは統一されました。考えられる鳴きが複数ある場合は、「チー」を押した後鳴きたい牌を
2枚選択してください。
...
- カーソル移動：矢印キー、HJKL（ローグライク方式）、WASD（FPS方式）
- 決定、打牌：Enter、Space、Z
- キャンセル：Esc、X
...
- 萬子の中張牌なし。三色同順は消滅。(数牌三麻を除く)
    ...
    - 瀬戸内三麻では萬子ではなく索子の中張牌を除いてプレイし、緑一色が消滅
- ドラ表示牌が一萬の時は、九萬がドラ(数牌三麻を除く)
    - 瀬戸内三麻ではそれぞれ一索、九索と読み替え
- チーなし
- 北は面子で使うと客風
...
牌譜はXML形式で出力され、対局を終えた日時がファイル名となります。
XML形式の牌譜を閲覧するには、「GL-MahjongTile」フォントをインストールしていただく必要があります。
...
なお、従来のテキスト形式、HTML形式の牌譜は廃止されました。
...
　バージョン (to be filled) より、牌譜用フォントデータ (WOFF) を追加しました。
SVG in OpenType フォントも同梱しており、Firefoxでは牌がカラーで表示されます。
