...
void sound::OggData::Prepare(const std::string& filename) {
	...
	// ファイルを開く
	FILE* file;
	if (fopen_s(&file, filename.c_str(), "rb")) throw "ファイルを開けませんでした";
	OggVorbis_File* ovFile(new OggVorbis_File());
	if (ov_open(file, ovFile, nullptr, 0)) {
		fclose(file); throw "Ogg Vorbis ファイルではありません";
	}
	// メタデータ読み込み
	const vorbis_info *info(ov_info(ovFile, -1));
	if (!info) throw "Ogg Vorbis フォーマットが取得できません";
	std::memset(&format, 0, sizeof(format));
	...
	do {
		...
		if (bytes_read == OV_HOLE) throw "読み込みはOV_HOLEで失敗しました";
		else if (bytes_read == OV_EBADLINK) throw "読み込みはOV_EBADLINKで失敗しました";
		else if (bytes_read == OV_EINVAL) throw "読み込みはOV_EINVALで失敗しました";
		else if (bytes_read)
