...
private:
	std::recursive_mutex myCS;
public:
	MHJMutex() = default;
	MHJMutex(const MHJMutex&) = delete;
	MHJMutex operator= (const MHJMutex&) = delete;
	~MHJMutex() = default;
	void acquire() {myCS.lock();} // ミューテックスを獲得
	bool tryAcquire() {return myCS.try_lock();} // ミューテックスを獲得(ロックしない)
	void release() {myCS.unlock();} // ミューテックスを解放
