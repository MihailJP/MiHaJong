...
#ifdef _WIN32
	if (lsock == INVALID_SocketDescriptor) throw socket_creation_error(WSAGetLastError()); // ソケットの作成に失敗したら例外
#else /* _WIN32 */
...
#ifdef _WIN32
	if (sock == INVALID_SocketDescriptor) throw socket_creation_error(WSAGetLastError()); // ソケットの作成に失敗したら例外
#else /* _WIN32 */
...
int mihajong_socket::Sock::client_thread::establishConnection() { // 接続を確立する
	...
	while (true) {
#ifdef _WIN32
		if (::connect(*mySock, (sockaddr*)&myAddr, sizeof(myAddr)) == SocketDescriptor_ERROR) { // 接続
...
int mihajong_socket::Sock::server_thread::establishConnection() { // 接続を確立する
	...
	if (::listen(*listenerSock, SOMAXCONN) == SocketDescriptor_ERROR) { // 待機
	...
	while (true) {
		...
		*mySock = accept(*listenerSock, nullptr, 0);
		if (*mySock == INVALID_SocketDescriptor) { // ソケットの作成に失敗場合
			errcode = WSAGetLastError();
