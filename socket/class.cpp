...
unsigned char mihajong_socket::Sock::getc () { // 読み込み(非同期)
	...
	{
		std::ostringstream o;
		o << "バイト受信 dequeue ポート [" << portnum << "] バイト [0x" <<
...
void mihajong_socket::Sock::putc (unsigned char byte) { // 書き込み
	{
		std::ostringstream o;
		o << "バイト送信 enqueue ポート [" << portnum << "] バイト [0x" <<
			std::hex << std::setw(2) << std::setfill('0') << (unsigned int)byte << "]";
		trace(o.str().c_str());
	}
	if (isServer) {
...
DWORD WINAPI mihajong_socket::Sock::network_thread::myThreadFunc() { // スレッドの処理
	...
	while ((!sender_closed) || (!receiver_closed)) { // 終了するまで
		...
		if (receive_ended) { // 終了ならソケットをシャットダウン
			std::ostringstream o; o << "受信ポートをシャットダウン ポート[" << myCaller->portnum << "]"; debug(o.str().c_str());
		...
		if (send_ended) { // 終了ならソケットをシャットダウン
			std::ostringstream o; o << "送信ポートをシャットダウン ポート[" << myCaller->portnum << "]"; debug(o.str().c_str());
			shutdown(*mySock, SD_SEND); sender_closed = true; send_ended = false;
		}
		Sleep(0);
	}
	{std::ostringstream o; o << "送受信スレッドループの終了 ポート[" << myCaller->portnum << "]"; debug(o.str().c_str());}
	finished = true;
	return S_OK;
}
...
void mihajong_socket::Sock::network_thread::wait_until_sent() { // 送信キューが空になるまで待つ
	{
		std::ostringstream o; o << "ポート [" << myCaller->portnum << "] の送信データはここまでらしい、全部送り終わるまで待ちます";
		debug(o.str().c_str());
	}
	...
	{
		std::ostringstream o; o << "ポート [" << myCaller->portnum << "] の送信は終わったんじゃないかな？";
		debug(o.str().c_str());
	}
}
